...
subtitle: "Ciblage des manques de complétude essentiels et consultation des données pour les régions Bretagne et Pays-de-la-Loire"
...
load(file = "data/outputs/bdroe_valid.RData")
```
...
* Jointure attributaire des informations de la BDOE et du ROE, avec suppression des doublons ;

* Création et renseignement des différents champs d'identification des lacunes attributaires de la base :
...
* Pour les ouvrages non gelés et non dérasés (`r nrow(bdroe %>% filter(statut_nom != "Gelé" & derasement_solde == 0))` ouvrages) : 
  ...
  * `r nrow(nb_manque_l2) %>% format(scientific=FALSE, big.mark=" ")` **ouvrages  situés en liste 2** (hors ouvrages de type 'Epis en rivière') manquent d'une de ces informations essentielles (`r round(nrow(nb_manque_l2)*100/nrow(nb_ouvrages_l2))`% des ouvrages situés en liste 2)
...
#### Synthèse des manques par masse d'eau
